...
Permite crear, seleccionar, clonar y eliminar modelos, además de gestionar las
...
la elimina de nuevo en cuanto queda vacía.

Para crear una carpeta, toque **+** junto a "Uncategorized" (o mantenga pulsado
...
card/eMMC. Al tocar el nombre de una carpeta se abre la opción de renombrar o
eliminar; al eliminar una carpeta, los modelos que contenga vuelven a
Uncategorized.

![Cambiar de carpeta](../assets/model-modelselect-folder-change-select.png)

Para mover un modelo, toque su icono, seleccione **Change folder** y luego toque
el destino:
...
Seleccione la categoría en la que desea crear el modelo, toque **+** y después
**Create model** para iniciar el asistente (cree primero la categoría si aún no
existe). Hay asistentes disponibles para **Airplane**, **Glider**,
**Helicopter**, **Multirotor** y **Other**; cada uno recorre la configuración
básica de ese tipo de fuselaje, incluidas mezclas preestablecidas opcionales
...
**AETR**: deje [Sticks → Orden de canales](../system-setup/controls.md) en su
valor predeterminado AETR con **First four channels fixed** activado, de modo que
...
1 profundidad + 1 dirección + 1 motor, el resultado es:
...
!!! tip
    ...
    funciones de estabilización siguen funcionando correctamente en cualquier
    caso, ya que la dirección de la compensación se establece durante la
    configuración de la estabilización. Si necesita ajustarse a la convención
    del manual del receptor, lo más sencillo es construir el modelo con el
    asistente de la forma habitual y después utilizar **Swap channels** en
    [Salidas](outputs.md) para intercambiar los dos canales de alerones; así se
    mantiene positivo el signo del diferencial del mezclador de alerones.
...
![Asistente: número de canales de profundidad/dirección](../assets/model-modelselect-model-wizard-ele-and-rudder.png)
...
La **configuración de la cola** ofrece la elección entre cola tradicional en
cruz, cola en V o sin cola (delta/ala volante):
...
- **Delta/ala volante**: al crear un modelo Airplane con 2 alerones y sin
  ...
  predeterminados del 50 % para que la aplicación simultánea a fondo de alerones
  + profundidad siga sumando el 100 %.
- **Delta con un receptor estabilizado que realiza la mezcla**: en su lugar
  seleccione 1 alerón y 1 profundidad; la mezcla de elevones se realiza en el
  receptor, según su propio manual.
- **Delta con superficies dedicadas de alerones y profundidad**: deje que el
  asistente funcione como si el modelo tuviera cola; configurará los canales de
  alerones y profundidad necesarios (con o sin dirección) y no se creará ninguna
  mezcla de elevones.

El paso de **reasignación de canales** permite anular la asignación
predeterminada del asistente, teniendo en cuenta que los receptores
estabilizados necesitan sus canales en un orden concreto (consulte las
instrucciones del propio receptor). El último paso establece el nombre del modelo
y vincula una imagen.

El modelo finalizado se guarda en la carpeta de categoría que estuviera activa
al iniciar el asistente, ordenado alfabéticamente dentro de ella. Consulte
[Ejemplo básico de ala fija](../tutorials/basic-fixed-wing.md) para ver un
recorrido completo desarrollado.

## Recibir un modelo desde otra emisora Ethos

![Recibir modelo](../assets/model-modelselect-model-receive.png)

Seleccione la categoría de destino, toque **+** y después **Receive model**: la
emisora queda a la espera y muestra su dirección Bluetooth para que el emisor
pueda localizarla. En la emisora que envía, toque el modelo y seleccione **Send
model**; la emisora receptora confirma el nombre del archivo entrante antes de
aceptarlo.

## Seleccionar un modelo

Toque **Model select** para acceder a la lista de modelos.
...
!!! note "Conversión de modelos tras una actualización de Ethos"
    ...
    *selecciona* después de una actualización de versión, no todos a la vez al
    ...
    momento posterior, incluso bajo una versión de Ethos aún más reciente. La
...
**Menú de gestión de modelos**: toque un modelo para resaltarlo y toque de nuevo
para abrir el menú:

- **Set current model**
- **Clone**: duplica el modelo. Un clon recibe automáticamente un nuevo número
  de receptor; si en su lugar reasigna el número de receptor del original,
  funciona sin necesidad de volver a hacer el binding.
- **Change folder**
- **Send**/**Receive**: hacia o desde otra emisora, como se ha descrito arriba.

...
la emisora en el momento de conectarla.

## Modo apagada

Conectar la emisora a un PC mediante USB **estando apagada** la pone en
modo DFU, utilizado para grabar el propio bootloader.

## Modo bootloader {: #bootloader-mode }

Encienda la emisora **manteniendo pulsado `ENT`** para arrancar en modo
bootloader (la pantalla muestra "Bootloader"). Al conectar el USB en este
momento, el estado cambia a "USB Plugged" y el PC monta **dos** unidades:
la memoria flash interna de la emisora y el contenido de la SD card/eMMC.
...
Suite](../ethos-suite/index.md) actualiza el firmware de la emisora —
consulte la sección Modo bootloader de Ethos Suite.

## Modo encendida

Conectar el USB con la emisora **encendida normalmente** muestra un
selector de modo:

- **Joystick** — presenta la emisora como un joystick USB HID, para manejar
  simuladores de vuelo en el PC.
- **FrSky Suite** — pone la emisora en "modo Ethos" para la comunicación

---
translated_from: 827e532e2b0324591f0fdbb61a39e61180642b24
---

# Guides pratiques

Guides courts et centrés sur une tâche précise, consacrés aux réglages que
les pilotes souhaitent couramment réaliser, par opposition à la référence
onglet par onglet de la [Configuration du modèle](../model-setup/index.md).

- [Alerte de tension de batterie basse](low-battery-warning.md)
- [Alerte de capacité de batterie](battery-capacity-warning.md)
- [Configuration du modèle SR8/SR10 et réorganisation des voies](sr8-sr10-setup.md)
- [Mixage papillon (corbeau)](butterfly-mixer.md)
- [Configurer un système FBUS](fbus-setup.md)
- [Tester une installation à récepteurs redondants](test-redundant-receiver.md)
- [Liste de vérification textuelle définie par l'utilisateur](user-defined-checklist.md)
- [Courbe de compensation réglable en vol](in-flight-compensation-curve.md)
- [Reprise instantanée pour la fonction écolage](instant-takeback.md)
- [Trouver le dernier bootloader ou un autre composant](find-latest-bootloader.md)

...
charge. Appuyez sur le **+** à côté de « Screen1 » pour en ajouter un autre :

- Choisissez parmi **15** dispositions, dont deux dispositions dédiées à
  l'écran d'accueil et une option plein écran, pouvant accueillir jusqu'à
  9 widgets — configurés exactement comme le premier écran.
- Les écrans peuvent être réordonnés ou supprimés depuis leur propre boîte
  de dialogue d'édition (appuyez sur Screen1, Screen2, etc.).

## Exemple concret

![Vue principale](../assets/display-main-view.png)

Une disposition typique : l'image du modèle (configurée dans [Édition du
...
taille de la grille de disposition, arrière-plan, et choix des écrans inclus
dans le cycle `PAGE`.

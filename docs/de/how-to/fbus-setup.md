...
F.Port2) legt Steuerung und Telemetrie auf eine gemeinsame Leitung, sodass sich
mehrere FBUS-Geräte eine einzige, in Reihe (Daisy-Chain) geschaltete Verbindung
teilen und vollständig drahtlos konfiguriert werden können. In dieser Anleitung
werden zwei Xact-Servos an den Querruderkanälen (1 und 5) des [grundlegenden
Beispiels für ein Flächenflugzeug](../tutorials/basic-fixed-wing.md)
angeschlossen.
...
FBUS setzt aktuelle Firmware sowohl im Empfänger als auch in den Geräten voraus
— Xact-Servos benötigen z. B. v2.0.1 oder neuer. Die entsprechenden Updates
finden Sie auf der
...
Firmware-Datei aus und anschließend **Flash External Device**.
...
nicht eines der beiden geändert wird. Je nach Empfängertyp gibt es zwei Wege:
...
2. Nur mit angeschlossenem Servo 2 (Pin5) das gleiche Menü aufrufen (Device
   ...
   ändern, **Save to flash**. Rufen Sie anschließend Device Config erneut auf,
...
Empfängers → **Options** → **Telemetry Port** auf **FBUS** setzen. Die
Xact-Servos werden dann in Reihe an diesen Port angeschlossen; da jedes Servo
nur einen Anschluss besitzt, verteilt ein F.Port2-Mehrkanal-Extender
(FP2CH4/6/8) das Signal auf mehrere Geräte.
...
jeder auf FBUS gesetzte Pin führt dasselbe FBUS-Signal.

## 5. FBUS-Steuerung der Servos prüfen

Servo 1 an Pin1 und Servo 2 an Pin5 anschließen (die Querruderkanäle des
Flächenflugzeug-Beispiels), einschalten und prüfen, ob die Kanäle 1 und 5 die
...
[Telemetrie](../model-setup/telemetry.md) und führen Sie die Sensorsuche erneut
durch. Jedes Servo meldet 4 Sensoren: Strom, Spannung, Temperatur und Status
(`OK` im Normalfall).
...
dieses Servos direkt geöffnet. Führen Sie nach jeder Änderung
**Save to flash** aus.

...
FrSky stabilized receivers can now be configured via ‘Device config’ after installation of the necessary setup Lua scripts. These are easily installed with 1 click from the Lua Library in ETHOS Suite, please refer to the [Lua library](../ethos-suite/operation.md) section.
...
**Note**: The older legacy receivers and the Archer & Archer Pro receivers use the SxR option. In addition receivers such as the SR10 Pro, even though named SRx instead of SxR and have Gain assigned to channel 9, use the SxR option. The newer receivers with "Advanced stabilization" and have the Gain control on channel 13 use the ‘Stabilizer config’ option.

Stabilizer config

The process has been streamlined, but will be immediately familiar if you've used the SxR or SRx Lua before.

![](../assets/Pictures/1000000000000320000001E052BC5E07.png)
...
Under the group 1 and 2 Calibration function, the Self-Check step has been replaced by a far superior independent calibration of aircraft level, channel center and channel endpoints. In addition, each channel can now be activated/deactivated.
...
FrSky North America have compiled [a comprehensive guide](https://docs.google.com/document/d/1...it?usp=sharing) to stabilized receiver setup which covers all the detail.

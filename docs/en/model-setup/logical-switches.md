...
Once logic switches have been defined, tapping on a logic switch will bring up the above popup menu, allowing you to edit, move, copy/paste, clone or delete that switch. You can also add a new logic switch by selecting ’Add’, or by tapping on the ‘+’ symbol next to the column headings.
...
The example above shows logic switch LS3 added, as well as its inverse !LS3 to ensure that the mix will always have valid values.
...
The above shows the mix summary line with the offset always having a valid value. The source has been set to the Special value 0, so the offset will be added to 0% and the mix output will be +100% when LS3 is active, or -100% when LS3 is inactive.

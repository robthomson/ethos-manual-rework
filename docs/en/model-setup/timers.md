...
In the main timers screen (see above) new timers may be added by tapping on the ‘+’ symbol next to the column headings.

![](../assets/model-timers-action-select.png)

Touching any timer line brings up a popup with options to reset or edit that timer, or to move or copy/paste the timer.
...
Select the type of audio action required, i.e. ‘Count seconds’ in the example above.

#### Start

The start value is the value from which this count down action starts.
...
Audio action types include ‘Count seconds’ (by voice), ‘Beep’ (with beeps instead of voice), ‘Play file’ and ‘Play value’.

![](../assets/model-timer1-actions-summary.png)

In this example above three audio actions have been configured:

1. Firstly a count seconds alert starting at 2 minutes remaining will be given every 30 seconds. The alert will be speech and haptic feedback has also been enabled.
2. Secondly a beep alert starting at 10 seconds remaining, after which a beep will be played every second. Haptic feedback has also been enabled.
...
1. Firstly a count down to the alarm value starting at 2 minutes remaining will be given every 30 seconds. The alert will be speech and haptic feedback has also been enabled.
2. Secondly a beep alert starting at 10 seconds remaining, after which a beep will be played every second. Haptic feedback has also been enabled.

...
|  | 040x | Temperature 1 |
|  | 041x | Temperature 2 |
| Xact Servo | 680x | Current, Voltage, Temp, Status |
...
- Discover new sensors (refer to [Telemetry](telemetry.md) Setup), and test carefully that Smart Port switching is working correctly.
...
When using TD or TW protocols, there is an option to receive Individual RSSI voice alerts per band.
...
![](../assets/Pictures/1000000000000320000001E05651F8DA.png)
...
Note that the minimum and maximum values are also defined for each parameter, even though they are not displayed on the sensor list. For example, when Altitude is defined, Altitude- and Altitude+ for the minimum and maximum altitude also become available.

Sensor discovery must be done for every model, and every time a new sensor is added.

![](../assets/model-telemetry-discover-new-sensors-off.png)

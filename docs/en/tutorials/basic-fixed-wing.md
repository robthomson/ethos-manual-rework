...
Click on 'Add a new weight' again, and set up a 30% Rate for switch SB in the down position. The ‘SB-’ is shown in bold, which means this is the current position. The vertical axis in the graph on the right now shows that only 60% of throw is available with the switch in this mid position. Note that the Rate will be 100% with switch SB in the up position.
...
Choose Pot S1 from the sources list. You can see the effect of Pot S1 in the graph on the right.
...
Please read through the next section on configuring the Channels before proceeding. To avoid damage by inadvertently over-driving your servos, it would be wise to disconnect your servo linkages or reduce the servo travel until you are ready to configure the servo min/max limits.

## Step 6. Configure the channels

The Channels section is the interface between the setup "logic" and the real world with servos, linkages and control surfaces, and motors or engines. So far we have set up the logic for what we want each control to do. Now, we can adapt that to the mechanical characteristics of the model. The various channels are outputs, for example CH1 corresponds to servo plug #1 on your receiver.

![](../assets/tut-fw-eg-outputs-icon.png)

Tap on the Channels icon to configure the output channels.

![](../assets/tut-fw-eg-outputs.png)

Tap on an output channel to configure it.

### Example 1: Aileron Left
...
Finally, you can use the Channel balancing feature in Channels to synchronize the movement of left and right surfaces such as ailerons and flaps. Please refer to the [Balance channels](#Balance channels) section.
...
This will open the Mixes Library. Select 'Free Mix'.

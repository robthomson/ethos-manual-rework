...
1. Leggere il tipo di radio, l'ID e le versioni installate — firmware,
   ...
   SD card/eMMC.
2. Commutare la radio tra la modalità bootloader e l'esecuzione di Ethos, e viceversa.
3. Confrontare le versioni installate con quelle correnti e aggiornarle automaticamente —
   solo i componenti obsoleti, tutto indipendentemente dallo stato, oppure i componenti
   singolarmente.
4. Eseguire il backup dei modelli su disco tramite **Model Manager**, o ripristinare un backup
   precedente (necessario poiché i file dei modelli non sono retrocompatibili tra le
   versioni del firmware).
5. Scaricare qualsiasi firmware dal sito di download FrSky tramite il **Download
   center**, e utilizzare la radio come proxy per aggiornare direttamente un modulo, un sensore,
   un servo o un ricevitore.
6. Convertire immagini e file audio nei formati nativi di Ethos.
7. Fornire **strumenti di sviluppo Lua** — documentazione API, script dimostrativi e un
   terminale di debug.
8. Aggiornare il bootloader della radio in modalità DFU (una connessione a radio spenta),
...
10. Espellere in modo sicuro le unità USB della radio.
11. Notificare all'avvio la disponibilità di un aggiornamento della Suite stessa (installato
    all'uscita).
...
- **Radio in modalità Bootloader** — la scheda **Radio** verifica/aggiorna il
  firmware e i file della flash/SD card/eMMC; **Model Manager** esegue il backup
  o il ripristino della radio.
- **Radio in modalità Ethos** — Suite utilizza la radio come proxy (tramite gli
  strumenti **FRSK Flasher**/Download center) per aggiornare direttamente il modulo interno,
  o qualsiasi sensore/servo/ricevitore collegato.
- **Radio in modalità DFU** — connessione a radio spenta, utilizzata dal **DFU
  Flasher** per aggiornare il bootloader stesso, ad esempio quando una corruzione del firmware
  impedisce alla radio di accendersi normalmente.

Vedi [Migrazione](migration.md) per trasferire per la prima volta una radio esistente a Ethos

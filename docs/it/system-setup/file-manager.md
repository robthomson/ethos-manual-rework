...
Gestione file consente di esplorare la memoria della radio e di aggiornare il
...
Toccare **Flash** (oppure premere `PAGE` per cambiare unità) per esplorare
l'unità flash USB virtuale interna della radio, utilizzata per le bitmap e i
font di sistema:
...
interna, in tutte le radio FrSky fino alla X9D originale.
...
— toccare **Radio** per esplorarla. Ethos crea automaticamente le cartelle
...
- **`bitmaps/`** — `bitmaps/models/` contiene le immagini dei modelli
  dell'utente (impostate in [Model Edit](../model-setup/model-edit.md) o nelle
  procedure guidate di creazione di un nuovo modello); `bitmaps/user/` contiene
  tutto il resto. Formato consigliato: BMP a 32 bit, 8 bit per colore, con
  canale alpha, 300×280px — in questo modo la decodifica a bordo della radio
  ...
  compaiono comunque in Gestione file, ma non risultano selezionabili lì. Gli
...
- **`Firmware/`** — file di firmware per il modulo RF interno, i moduli esterni
  ...
  Copiare qui il nuovo firmware mentre la radio è in [modalità
...
- **`models/`** — i file dei modelli veri e propri. Non possono essere
  ...
  diventa `Extra.bin`; un secondo "Extra" diventa `Extra01.bin`). Rinominare un
  modello in [Model Edit](../model-setup/model-edit.md) rinomina anche il
  relativo file — sempre in caratteri minuscoli (il nome visualizzato con
  maiuscole e minuscole è memorizzato all'interno del file), e non tutti i
...
- **`scripts/`** — script Lua, eventualmente organizzati in sottocartelle
  ...
  (vedere [Display](../displays/index.md)), i **task e le sorgenti** (sensori
...
- **`radio.bin`** (cartella principale) — il file delle impostazioni di sistema,
  scritto dalla radio stessa in fase di inizializzazione. Eseguirne il backup
  insieme a `models/` prima di un aggiornamento del firmware, in modo da poter
  eventualmente tornare a una versione precedente.

- **`firmware.bin`** (cartella principale) — copiare qui un nuovo file di
  firmware della radio per farlo installare automaticamente alla successiva
...
**ricevente**, spostarsi nella cartella di destinazione in Gestione file,
premere a lungo `ENT` e scegliere **Receive file here**:

![Ricezione Bluetooth](../assets/system-filemanager-bluetooth-receive.png)

Sulla radio **trasmittente**, toccare il file, scegliere **Send file** e seguire
le indicazioni su entrambe le radio:

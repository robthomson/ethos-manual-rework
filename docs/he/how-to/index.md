---
translated_from: 827e532e2b0324591f0fdbb61a39e61180642b24
---

# מדריכי הדרכה

מדריכים קצרים וממוקדי-משימה עבור דברים ספציפיים שטייסים נוהגים להגדיר, בשונה מהמידע העיוני לפי לשוניות שמופיע ב[הגדרות דגם](../model-setup/index.md).

- [אזהרת מתח סוללה נמוך](low-battery-warning.md)
- [אזהרת קיבולת סוללה](battery-capacity-warning.md)
- [הגדרת דגם ושינוי סדר ערוצים ב-SR8/SR10](sr8-sr10-setup.md)
- [מיקסר Butterfly (עורב)](butterfly-mixer.md)
- [הגדרת מערכת FBUS](fbus-setup.md)
- [בדיקת מערך מקלטים מיותר (גיבוי)](test-redundant-receiver.md)
- [רשימת בדיקה עם טקסט מוגדר על ידי המשתמש](user-defined-checklist.md)
- [עקומת פיצוי הניתנת לכיול בזמן טיסה](in-flight-compensation-curve.md)
- [החזרת שליטה מיידית עבור פונקציית המאמן](instant-takeback.md)
- [איתור ה-Bootloader העדכני או רכיב אחר](find-latest-bootloader.md)
